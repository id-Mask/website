...
for (const country of Object.keys(countryAlpha2codeToNumericMap)) {
    ...
    */
    ...
    const concatUnicode = letterToUnicodeMap[firstChar] * 100 + letterToUnicodeMap[secondChar];
    concatUnicodeToNumericMap[concatUnicode] =
        countryAlpha2codeToNumericMap[country];
}
const countryNumericToAlpha2codeMap = {};
...
for (const concatUnicode in concatUnicodeToNumericMap) {
    map.set(Field(concatUnicodeToNumericMap[concatUnicode]), Field(concatUnicode));
}
console.log(letterToUnicodeMap, countryAlpha2codeToNumericMap, concatUnicodeToNumericMap, countryNumericToAlpha2codeMap, map);
export { countryNumericToAlpha2codeMap };
//# sourceMappingURL=ProofOfNationality.utils.js.map